...
const Footer = (props) => {
  ...
  const handleWaitingClick = () => {
    ...
    if (!isWaiting) {
      props.setShouldAutoPlayNext(true);
    }
  };
      ...
      <AppBar postion="sticky" className={classes.appBar}>
        <Toolbar className={classes.toolbar}>
          ...
          <Typography variant="h6" className={classes.title} noWrap>
            {props.currentlyPlaying === "waiting"
              ? "Waiting for next audio clip..."
              : (props.currentlyPlaying === null ? "Livestream" : "Playing audio clip")}
          </Typography>
          <FormControlLabel
            value="top"
            control={
              <Checkbox
                ...
                onChange={() => props.setShouldAutoPlayNext(!props.shouldAutoPlayNext)}
              />
            }
            label={"autoplay"}
